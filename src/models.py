...
class SmoothSVM(object):
    ...
    def fit(self, X, Y):
        ...
        if self.fit_intercept == True:
            ...
            p = X.shape[1] - 1 # substract 1 for the column of ones
...
def calculate_errors(n, p, mu1, mu2, Sigma, n_iter, results_dict):
    ...
    for i in range(1, n_iter):
        ...
        results_dict["svc_errors"][n].append(
            np.linalg.norm(np.concatenate([svc.intercept_, svc.coef_[0]]).reshape(-1, 1) - psvm.coefs))
...
# These functions are used for an adjustment so that the L2 norm of the coef vector
# does not scale with p.


def calculate_adjustment_constant(c, mu1_base, mu2_base, Sigma_base):
    d = mahalanobis_distance(mu1_base, mu2_base, Sigma_base)
    a = solve_gamma_equation(d / 2)

    constant = solve_gamma_equation2(a, c, d)
...
def mahalanobis_distance(mu1, mu2, Sigma):
    mu_diff = mu1 - mu2
    Sigma_inv = np.linalg.inv(Sigma)
